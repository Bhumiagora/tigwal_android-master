...
class ProfileActivity : BaseActivity(), KodeinAware, View.OnClickListener {
    ...
    private fun openCamera() {
        ...
        filename = folder.getAbsolutePath() + "/" + System.currentTimeMillis() + ".jpg"
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode === PICK_CAMERA_REQUEST && resultCode === Activity.RESULT_OK) {
            ...
            if (addresses!!.size > 0) {
                strselectedCountry = (addresses?.get(0)?.getCountryName() ?: binding.txtCountry.setText(strselectedCountry)).toString()
